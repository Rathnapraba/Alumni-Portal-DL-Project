...
<div class="text-center">
    ...
    <a href="{{ url('/alumnis/create') }}" class="btn btn-success btn-sm" title="Add New Job Post">
        <i class="fa fa-plus" aria-hidden="true"></i> Post Job/Internship
    </a>

    <br /><br />

    <a href="{{ url('/alumnis/blog') }}" class="btn btn-success btn-sm" title="Add New Job Post">
        <i class="fa fa-plus" aria-hidden="true"></i> Post Blog
    </a>

    <br /><br />

    <a href="{{ url('/alumnis/event') }}" class="btn btn-success btn-sm" title="Add New Job Post">
...
@foreach($alumnis as $index => $alumni)
    <div class="job-post">
        <h3>{{ $alumni->role }}</h3>
        <p><strong>Company Name:</strong> {{ $alumni->company_name }}</p>
        <p><strong>Location:</strong> {{ $alumni->location }}</p>
        <p><strong>Batch:</strong> {{ $alumni->batch }}</p>
        <p><strong>Description:</strong> {{ $alumni->job_description }}</p>

        <div class="actions">
            <a href="{{ url('/alumnis/' . $alumni->id . '/edit') }}" title="Edit Job Post" class="btn btn-primary btn-sm">
                <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit
            </a>

            <form method="POST" action="{{ url('/alumnis/' . $alumni->id) }}" accept-charset="UTF-8" style="display:inline">
                {{ method_field('DELETE') }}
                {{ csrf_field() }}
    ...
    <div class="comment-section">
        ...
        <button type="button" class="btn btn-primary submit-comment" data-post-id="{{ $index }}">Submit Comment</button>
        <div class="comments-container" data-post-id="{{ $index }}"></div>
    </div>
    
    
    <div id="sentiment-counts-{{ $index }}">
        <p>Total Likes: <span id="like-count-{{ $index }}">0</span></p>
        <p>Total Dislikes: <span id="dislike-count-{{ $index }}">0</span></p>
        <p>Total Hearts: <span id="heart-count-{{ $index }}">0</span></p>
    </div>
@endforeach
